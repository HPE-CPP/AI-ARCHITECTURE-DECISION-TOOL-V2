        ...
        <div className="lg:col-span-8 space-y-10 min-w-0">
          ...
          {result.followup_questions && result.followup_questions.length > 0 && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="glass-panel p-6 sm:p-10 rounded-[2rem] relative overflow-hidden group shadow-lg border-[color:var(--border)]"
            >
              {/* Internal code remains exactly the same... */}
              {/* Omitted for brevity to save space, copy from your previous working state */}
            </motion.div>
          )}
        </div>
